...
const inputS: React.CSSProperties = {
  ...
  padding:"16px", borderRadius:"14px", color:"white", fontSize:"14px",
...
export default function GeofencingClient({ org }: { org: any }) {
  ...
  return (
    <div style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.05)",borderRadius:"24px",padding:"32px",display:"flex",flexDirection:"column",gap:"24px",boxShadow:"0 10px 40px rgba(0,0,0,0.2)"}}>
      <div style={{background:"rgba(59, 130, 246,0.05)",border:"1px solid rgba(59, 130, 246,0.15)",borderRadius:"16px",padding:"20px"}}>
        <p style={{fontSize:"14px",color:"rgba(59, 130, 246,0.9)",fontFamily:"var(--font-inter)",lineHeight:1.6,fontWeight:500}}>
          ...
          {!org?.lat && <span style={{display:"block",marginTop:"8px",color:"rgba(255,255,255,0.3)"}}>Sin configurar — Pueden fichar desde cualquier lugar.</span>}
        </p>
      </div>
      ...
      <button onClick={detect} disabled={detecting}
        style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"14px",padding:"16px",color:"#FAFAFA",fontSize:"14px",fontFamily:"var(--font-inter)",fontWeight:600,cursor:"pointer",transition:"all 0.2s",opacity:detecting?0.6:1,display:"flex",alignItems:"center",justifyContent:"center",gap:"8px"}}>
      ...
      <div>
        ...
        <input type="range" min="50" max="1000" step="50" value={radius} onChange={e=>setRadius(e.target.value)}
          style={{width:"100%",accentColor:primary,height:"6px",background:"rgba(255,255,255,0.1)",borderRadius:"10px",outline:"none"}} />
        <div style={{display:"flex",justifyContent:"space-between",fontSize:"12px",color:"rgba(255,255,255,0.3)",marginTop:"8px",fontFamily:"var(--font-inter)",fontWeight:500}}>
          <span>50m</span><span>1000m</span>
        </div>
      </div>

      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:"12px",borderTop:"1px solid rgba(255,255,255,0.05)",paddingTop:"24px"}}>
        ...
        <button onClick={save} disabled={saving}
          style={{background:`linear-gradient(135deg,${primary},var(--accent-dark))`,color:"#000",padding:"16px 32px",borderRadius:"14px",fontFamily:"var(--font-inter)",fontWeight:700,fontSize:"14px",border:"none",cursor:"pointer",opacity:saving?0.6:1,transition:"all 0.2s",boxShadow:"0 10px 30px rgba(59, 130, 246,0.3)"}}>
